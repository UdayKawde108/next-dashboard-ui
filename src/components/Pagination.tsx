...
const Pagination = ({page,count}:{page:number;count:number}) => {
  ...
  const router=useRouter(); const hasPrev =ITEM_PER_PAGE*(page-1)>0;const hasNext =ITEM_PER_PAGE*(page-1)+ITEM_PER_PAGE<count;
  ...
  return (
    <div className='p-4 flex items-center justify-between text-gray-500'>
        ...
        <div className="flex items-center gap-2 text-sm">
          {Array.from({length:Math.ceil(count/ITEM_PER_PAGE)},(_,index)=>{const pageIndex=index+1;
